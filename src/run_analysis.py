...
from utils.config import DEFAULT_SUBJECTS_DIR, HEMISPHERES, VOL2SURF_FILENAME
from utils.utils import read_patients_from_excel, run_command, setup_logger
...
def resolve_fsgd(args: argparse.Namespace, output_dir: str, logger: logging.Logger) -> str:
    ...
    # validate() raises ValueError with all problems; caught by the entry point.
    validate(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f'Root directory containing FreeSurfer subject folders. Default: {DEFAULT_SUBJECTS_DIR}',
    )
...
def setup_analysis_logger(output_dir: str) -> tuple[logging.Logger, str]:
    os.makedirs(output_dir, exist_ok=True)
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    log_path = os.path.join(output_dir, f'analysis_{timestamp}.log')

    logger = setup_logger('run_analysis', log_path, file_mode='w')
...
if __name__ == '__main__':
    args = parse_args()
    logger, log_path = setup_analysis_logger(args.analysis_dir)
    try:
